...
const char * const BITCOIN_CONF_FILENAME = "compass.conf";
const char * const BITCOIN_PID_FILENAME = "compassd.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "compass" is a composite category enabling all Compass-related debug output
            if(ptrCategory->count(string("compass"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\CompassCoin
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\CompassCoin
    // Mac: ~/Library/Application Support/CompassCoin
    // Unix: ~/.compasscoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "CompassCoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/CompassCoin";
#else
    // Unix
    return pathRet / ".compasscoin";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty compass.conf if it does not excist
