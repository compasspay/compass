 ...
 *
 *   // network
 *   CCompassNetwork lvl, network-type, network-status, network-error, milestone-status*
...
private:
    ...
    // Current OBJECT STATUS (see http://govman.compass.org/index.php/Documentation_:_Status_Field)
    int nStatusID;
...
// // root node
class CCompassNetwork : public CGovernanceObject
{
private:
    std::string strName;
    std::string strURL;
...
public:
    CCompassNetwork(UniValue objIn)
    {
...
// // can be under: CompassNetwork
// //   -- signature requirements : Key1(User)
// class CCompassNetworkVariable : public CGovernanceObject
// {
// private:
